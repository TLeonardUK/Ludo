...
)
{
	...
	for (const Platform::Path& path : paths)
	{
		if (std::find(validExtensions.begin(), validExtensions.end(), path.GetExtension()) != validExtensions.end())
		{
			...
			info.SourceSourcePath = path.ChangeExtension("cpp");
			...
			info.GeneratedHeaderPath	= outputDirectory + relativePath.ChangeExtension("generated.h");
			info.GeneratedSourcePath	= outputDirectory + relativePath.ChangeExtension("generated.cpp");
			info.ManifestPath			= info.GeneratedHeaderPath.ChangeExtension("generated.manifest");
			info.bOutOfDate				= false;
			info.Hash					= CalculateFileHash(info.SourcePath);
			info.CodePrefix				= "LD_" + CastToString(info.Hash) + "_";
			info.IncludePaths			= includePaths;
			info.Defines				= defines;

			info.IncludePaths.push_back(outputDirectory);

			Platform::Path baseDirectory = info.GeneratedHeaderPath.GetDirectory();
			if (!baseDirectory.Exists())
			{
				baseDirectory.CreateAsDirectory();
			}

			if (!bDoNotCalculateOutOfDate)
			{
				if (!info.ManifestPath.Exists() ||
					!info.GeneratedHeaderPath.Exists() ||
					!info.GeneratedSourcePath.Exists())
				{
