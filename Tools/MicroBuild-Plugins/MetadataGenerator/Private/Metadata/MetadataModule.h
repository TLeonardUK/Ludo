...
public:
	...
	// Tries to find a class by its internal clang cursor.
	MetadataClass::Ptr FindClassByCursor(CXCursor cursor);

	// Tries to find an unspecialized class template by its internal clang cursor.
	MetadataClass::Ptr FindUnspecializedTemplateClassByCursor(CXCursor cursor);

	// Tries to find class by its fully qualified name.
	MetadataClass::Ptr FindClassByName(const std::string& name);
