...
{
public:
	ClangAstVisitor(Platform::Path sourcePath, CXTranslationUnit translationUnit);
...
public:
	...
	// Helper function, gets a cursors file info without having to deal with the clang memory management.
	...
	bool ResolveType(CXType type, MetadataType::Ptr& OutValue);

	// Attempts to find or create a metadata class based on a clang type. Retusn true on success. 
	bool ResolveClass(CXType type, MetadataClass::Ptr& OutValue);
...
protected:
	...
	bool AnalyseMethod(MetadataMethod::Ptr object);
	bool AnalyseField(MetadataField::Ptr object);
