...
extern WmiConnection gWmiConnection;

const char* NewLineSequence = "\r\n";
...
int GetCpuCount()
{
	...
	return Value.Get([]()
	{
		WmiQuery Query(&Environment::gWmiConnection, "select * from Win32_Processor");
...
int GetGpuCount()
{
	...
	return Value.Get([]()
	{
		WmiQuery Query(&Environment::gWmiConnection, "select * from Win32_VideoController");
		WmiQueryResult Result;
...
{
	static CacheArray<CpuInfo> Value;
