...
void LogLastSystemError(String SourceFunction)
{
	...
	LD_LOGF(Platform, Error, "{FunctionName} failed with system error {ErrorNumber} ({ErrorDescription}).", Args);
}
...
void AssertLastSystemError(String SourceFunction)
{
	...
	LD_LOGF(Platform, Error, "{FunctionName} failed with system error {ErrorNumber} ({ErrorDescription}).", Args);
	LD_CONSTANT_ASSERT_MSGF("{FunctionName} failed with system error {ErrorNumber} ({ErrorDescription}).", Args);
}
